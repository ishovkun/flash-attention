    ...
    for (int i = 0; i < Tr; i++) {
      ...
      for (int ii = 0; ii < Br; ii++) {
        for (auto k = tx; k < dp; k += warpSize) {
          ...
          Qi[ii * dp + k] =
              inBounds ? Q[qkv_offset + (tile_size * i) + ii * d + k] : 0.f;
        }
      }
      __syncthreads();
      ...
      wmma::store_matrix_sync(Sij, s_frag, wmma::WMMA_M, wmma::mem_row_major);
      ...
      for (int x = 0; x < d; x += wmma::WMMA_M) {
        wmma::fill_fragment(pv_frag, 0.0f);
        for (int k = 0; k < Bc; k += wmma::WMMA_K) {
